...
function Summary() {
  ...
  useEffect(() => {
    if (!quiz?.questions.length) navigate('/');
  }, [quiz, navigate]);

  const score =
    quiz.score ||
    quiz.questions.reduce((acc, curr) => {
      if (curr.correctAnswerIdx === curr.selectedAnswerIdx) return acc + 1;
      return acc;
    }, 0);

  const reportQuiz = async () => {
    try {
      const rawResponse = await fetch(endpoints.addQuiz, {
        method: 'POST',
  ...
  return (
    <div className="bg-background min-h-screen px-4 py-10 flex flex-col items-center gap-8">
      <Heading
        label={`You scored ${score}/${quiz.questions.length}`}
        customStyle="text-3xl text-primary font-bold text-center"
      />
      ...
      {!fromHistory && (
        <Button
          customCallback={reportQuiz}
          label="Report Quiz"
          customStyles="bg-wrong text-white font-semibold px-6 py-3 rounded-xl shadow-md hover:bg-red-600 transition w-full max-w-3xl"
        />
      )}
    </div>
  );
